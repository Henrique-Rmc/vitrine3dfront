...
export default function LojaTemplate({
  storeId,
  storeName,
  storeDescription,
  whatsappNumber,
  ...
  useEffect(() => {
    ...
    if (!typesWithProducts.some((t) => t.id === selectedTypeId)) setSelectedTypeId(null)
  ...
  const activeAttributes = useMemo(() => readActiveAttributes(searchParams), [searchParams])
  ...
  function getActive(key: string) { return searchParams.get(`attr_${key}`) ?? null }
  ...
  function clearAllFilters() { setSearchParams({}, { replace: true }) }
    ...
    <div style={themeToStyle(storeTheme)}>
              ...
              <div className="flex items-center gap-2 py-2.5 overflow-x-auto scrollbar-none">
                ...
                {Object.entries(activeAttributes).map(([key, value]) => (
                  <span key={key} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface-2 border border-border text-xs font-medium text-ink-2 shrink-0">
                    ...
                    {value}
                    <button onClick={() => setFilter(key, null)} className="ml-0.5 hover:text-ink transition-colors" aria-label={`Remover filtro ${key}`}>
            ...
            <div className="space-y-4">
              {Array.from(attributeMap.entries()).map(([key, { label, values }]) => (
                <div key={key} className="flex items-start gap-4">
                  <span className="text-xs font-semibold uppercase tracking-wide text-ink-3 pt-1.5 w-24 shrink-0">{label}</span>
                  <div className="flex flex-wrap gap-1.5">
                    <button onClick={() => setFilter(key, null)} className={chip(getActive(key) === null)}>Todos</button>
                    {values.map((value) => {
                      ...
                      return (
                        <button key={value} onClick={() => setFilter(key, value)} className={chip(getActive(key) === value)}>
                          {isBool ? (value === 'true' ? 'Sim' : 'Não') : value}
                        </button>
                      )
                    })}
                  </div>
              ...
              {activeCount > 0 && (
                <button onClick={clearAllFilters} className="text-xs text-ink-3 hover:text-ink transition-colors mt-1">
      ...
      {showAttrChips && filterPanelOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-[1px]" onClick={() => setFilterPanelOpen(false)} />
          <div className="absolute bottom-0 left-0 right-0 bg-canvas rounded-t-2xl max-h-[80vh] flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-border shrink-0">
              ...
              <div className="flex items-center gap-3">
                {activeCount > 0 && (
                  <button onClick={clearAllFilters} className="text-xs text-ink-3 hover:text-ink transition-colors">Limpar todos</button>
                )}
                <button onClick={() => setFilterPanelOpen(false)} aria-label="Fechar filtros" className="p-1 rounded-md text-ink-3 hover:text-ink transition-colors">
            ...
            <div className="overflow-y-auto flex-1 px-5 py-5 space-y-6">
              {Array.from(attributeMap.entries()).map(([key, { label, values }]) => (
                <div key={key}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-ink-3 mb-3">{label}</p>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {loading
              ? Array.from({ length: SKELETON_COUNT }).map((_, i) => <ProductSkeleton key={i} />)
