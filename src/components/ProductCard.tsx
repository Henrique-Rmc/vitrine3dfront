  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-surface-2">
        ...
        <button
          onClick={handleShare}
          className="absolute top-2 right-2 w-9 h-9 flex items-center justify-center rounded-full bg-black/30 hover:bg-black/50 text-white shadow-sm transition-colors z-10"
          aria-label={copied ? 'Link copiado!' : 'Compartilhar produto'}
          title={copied ? 'Link copiado!' : 'Compartilhar'}
        >
          {copied ? <ShareCheckIcon /> : <ShareLinkIcon />}
        </button>
      </div>
        ...
        <div className="flex items-center justify-between gap-2 mt-auto pt-1">
          ...
          {isAffiliate ? (
            <button
              ...
              aria-label={`Comprar no Site - ${name}`}
            >
              <ExternalIcon />
              Comprar no Site
            </button>
